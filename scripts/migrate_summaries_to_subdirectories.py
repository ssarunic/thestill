...
"""
Migration script to reorganize summary files from flat structure to podcast subdirectories.

Old structure: data/summaries/podcast-slug_episode-slug_hash_summary.md
New structure: data/summaries/podcast-slug/episode-slug_hash_summary.md

This script:
1. Reads all episodes from SQLite database
2. For each episode with summary_path:
...
Usage:
    python scripts/migrate_summaries_to_subdirectories.py --dry-run    # Preview changes
    python scripts/migrate_summaries_to_subdirectories.py --backup     # Backup before migration
    python scripts/migrate_summaries_to_subdirectories.py              # Run migration
"""
...
def backup_database(db_path: Path) -> Path:
    ...
    backup_path = db_path.parent / f"{db_path.stem}_backup_summaries_{timestamp}{db_path.suffix}"
...
def get_new_filename(old_filename: str, podcast_slug: str) -> str:
    """
    Convert flat filename to just the episode part.

    Old: podcast-slug_episode-slug_hash_summary.md
    New: episode-slug_hash_summary.md
    """
...
def get_file_moves(
    ...
    for row in cursor.fetchall():
        ...
        current_value = row[3]  # summary_path
        ...
        # Build full paths
        old_path = summaries_dir / current_value
        ...
        # Build new path
        new_dir = summaries_dir / podcast_slug
...
def run_migration(
    ...
    logger.info(f"Starting summary subdirectory migration")
    ...
    try:
        # Build file mapping
        logger.info("Analyzing summary files to move...")
        moves, stats = get_file_moves(conn, storage_path)

        logger.info(f"Summary files found in database: {stats.files_found}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate summary files from flat structure to podcast subdirectories",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scripts/migrate_summaries_to_subdirectories.py --dry-run     Preview changes
  python scripts/migrate_summaries_to_subdirectories.py --backup      Backup and migrate
  python scripts/migrate_summaries_to_subdirectories.py               Run migration
        """,
    )
    parser.add_argument(
