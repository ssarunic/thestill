...
"""Tests for the narration artefact GET endpoints (spec #33 Phase 3).

The user-visible POST trigger lives at
``POST /api/briefings/{briefing_id}/narrate`` (see test_api_briefings.py);
the endpoints here are for direct artefact access by TTS consumers.
"""
...
def _make_briefing(briefing_id: str, user_id: str) -> Briefing:
    ...
    return Briefing(
        id=briefing_id,
        user_id=user_id,
        cursor_from=datetime(2026, 5, 5, tzinfo=timezone.utc),
        cursor_to=now,
        episode_count=1,
        created_at=now,
    )
...
def mock_app_state(storage, mock_user):
    ...
    state.briefing_repository.get = lambda bid: briefings.get(bid)
    return state
