...
| `NARRATION_ANCHOR_PROMPT` | Anchor voice file under `thestill/services/narration_prompts/` (basename): `conversational_anchor` (one narrator talking to a friend), `conversational_v2` (same narrator with a point of view, reaction blocks, one claim per show) or `newsroom_anchor` (measured news-anchor). Spec #77 | `conversational_anchor` |
...
| `NARRATION_MATERIAL_MAX_WORDS` | Per-episode cap on the claim plus colour (one Key Takeaway, one Drama round) fed to the writer. Spec #77 | `400` |
