...
| `/api/status/mcp` | GET | Remote MCP connector info (spec #78): capability URL for claude.ai custom connectors; admin-gated |
...
- Remote MCP connector info: `GET /api/status/mcp` (spec #78 — returns the capability URL, which is operator-equivalent access)
