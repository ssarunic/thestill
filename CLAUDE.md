...
# Briefings (per-user, spec #36/#50) are generated by the web server:
# lazily on inbox open, or scheduled per user (BRIEFING_SCHEDULER_ENABLED).
# Narrate an existing briefing from the CLI:
thestill narrate --briefing <briefing-id>
...
├── briefings/             # Per-user briefing scripts
