...
| 46 | [web-usage-analytics](46-web-usage-analytics.md) | 📝 Draft (2026-05-22) | Web-only product analytics via PostHog, for the operator (not a user feature). Off-by-default for self-host: the PostHog project key is delivered at runtime through `/auth/status` and only exists when `POSTHOG_KEY` is set (hosted only), so `posthog-js` never initializes without it. Manual `$pageview` + an explicit, no-PII event taxonomy (play/pause/complete via PlayerContext, search, entity, digest, import, follow, inbox actions); identify by opaque `user.id`. Autocapture/session replay off; CSP widened only when keyed. Funnels/retention/feature-usage dashboards live in PostHog. No CLI analytics, no licensing work |
