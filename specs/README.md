...
| 52 | [inbox-reader-overlay](52-inbox-reader-overlay.md) | 📝 Draft (2026-07-07) | Open inbox episodes in a reader overlay above the still-mounted inbox list (Gmail-thread model) instead of a full route change into the Podcasts hierarchy. React Router background-location pattern keeps the canonical episode URL (refresh/share degrade to the standalone page, no duplicate URLs); Esc/scrim/`← Inbox` = `history.back()` restoring scroll + poll + query state. Requires extracting a shared `EpisodeReader` from `EpisodeDetail` and scroll-container-aware `useReadingPosition`. Frontend-only; Phase 2 adds read/save/dismiss buttons (completing #29 Phase 3) and j/k triage |
