...
> **Status:** 🚧 Implemented on `feat/60-refresh-network-failure-classification` (2026-07-22, "C+" design — v2 with review corrections: single policy-applying write with idempotent worker fallback, typed `error_class` on `ThestillError`, `RefreshPolicySettings` passed into the repo, `refresh_failure_streak_started_at` wall-clock horizon column, remote 5xx attributed `item` not `infra`). Phase 0 rollout (live-DB re-arm) intentionally NOT executed — operator's call.
> **Created:** 2026-07-22
