...
| B | `itunes:author` (already stored as [`Podcast.author`](../thestill/models/podcast.py#L361)) | feed | ~50/50 host name vs. network name | Free, already ingested |
...
title/description + episode title/description. Output: `hosts`, `guests`,
each `"Name - Role/Description"` to match the existing facts string format.
...
- **Merge rules** (highest wins): user edit > transcript-confirmed >
  `podcast:person` > LLM-metadata > `itunes:author`.
...
- **Web UI:** episode cards/inbox may display metadata-provenance guests with a
  subtle "expected" treatment before processing completes. Optional; not a
  blocker for the pipeline work.
...
- Should Tier-B `itunes:author` ever seed a host prior on its own? (Proposal:
  only when Tier A/C/D produced nothing, and always marked metadata-provenance —
  the network-name false-positive rate is too high to rank it higher.)
- Does the multi-user inbox want per-user visibility rules for unconfirmed
  guests, or is a global "expected" badge fine? (Assume global for now.)
