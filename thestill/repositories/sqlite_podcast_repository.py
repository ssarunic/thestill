...
from .podcast_repository import EpisodeRepository, PodcastRepository, RefreshOnOpenState


def _iso_to_utc(value: Optional[str]) -> Optional[datetime]:
    """Parse an optional ISO-8601 column to tz-aware UTC (naive → assumed UTC)."""
    if not value:
        return None
...
class SqlitePodcastRepository(PodcastRepository, EpisodeRepository):
    ...
    def get_refresh_on_open_state(self, podcast_id: str) -> Optional[RefreshOnOpenState]:
        """Spec #74 — see interface. ISO-8601 text columns are parsed to
        tz-aware UTC so the caller's interval arithmetic never mixes zones
        (spec #42 FM-3)."""
        with self._get_connection() as conn:
            row = conn.execute(
                "SELECT last_refresh_at, refresh_retry_after_at, refresh_disabled_reason FROM podcasts WHERE id = ?",
                (podcast_id,),
            ).fetchone()
        if row is None:
            return None
        return RefreshOnOpenState(
            last_refresh_at=_iso_to_utc(row["last_refresh_at"]),
            refresh_retry_after_at=_iso_to_utc(row["refresh_retry_after_at"]),
    ...
    def has_followers(self, podcast_id: str) -> bool:
        ...
        with self._get_connection() as conn:
            row = conn.execute(
                "SELECT 1 FROM podcast_followers WHERE podcast_id = ? LIMIT 1",
                (podcast_id,),
            ).fetchone()
            return row is not None
    ...
    # Mirrors the Postgres implementations; datetime columns come back as
    # ISO strings here, which every consumer (Pydantic models, JSON
    # serialization) parses transparently.
    # ------------------------------------------------------------------
    ...
    _EPISODE_COUNTS_SELECT = """
        ...
        (SELECT COUNT(*) FROM episodes e
          WHERE e.podcast_id = p.id AND e.failed_at_stage IS NULL
    ...
    def resolve_podcast_slug(self, slug: str) -> Optional[str]:
        if not slug:
            return None
        with self._get_connection() as conn:
            row = conn.execute("SELECT id FROM podcasts WHERE slug = ?", (slug,)).fetchone()
            return row["id"] if row else None
    ...
    def _podcast_row_from_db(self, row) -> Dict:
        ...
        return {
            ...
            "author": row["author"],
            "explicit": bool(explicit) if explicit is not None else None,
            "show_type": row["show_type"],
            "website_url": row["website_url"],
    ...
    def list_podcast_rows(
        ...
        *,
        ...
        if podcast_ids is not None:
            if not podcast_ids:
                return [], 0
            ids = [str(pid) for pid in podcast_ids]
            clauses.append(f"p.id IN ({','.join('?' * len(ids))})")
            params.extend(ids)
        needle = (q or "").strip()
        if needle:
            clauses.append("(LOWER(p.title) LIKE ? OR LOWER(COALESCE(p.author, '')) LIKE ?)")
            like = f"%{needle.lower()}%"
            params.extend([like, like])
        ...
        with self._get_connection() as conn:
            ...
            rows = conn.execute(
                f"""
                SELECT {self._PODCAST_ROW_COLS},
                       {self._EPISODE_COUNTS_SELECT}
                  FROM podcasts p
                  {where}
                 ORDER BY p.created_at DESC
                 {page}
