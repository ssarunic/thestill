...
class SqliteEntityRepository(EntityRepository):
    ...
    def replace_aliases(self, entity_id: str, aliases: List[str]) -> bool:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                ...
                (json.dumps(cleaned), datetime.now(timezone.utc).isoformat(), entity_id),
            )
    ...
    def list_alias_evidence(self, since: datetime) -> List[AliasEvidence]:
        sql = """
            WITH al AS (
                SELECT e.id, e.type, e.canonical_name, a.value AS alias
                FROM entities e, json_each(e.aliases) AS a
            )
            SELECT al.id, al.type, al.canonical_name, al.alias,
                COUNT(m.id) FILTER (WHERE m.resolution_method = 'direct' AND m.resolved_at >= ?) AS direct_since,
                COUNT(m.id) FILTER (
                    WHERE m.resolution_method = 'direct' AND (m.resolved_at < ? OR m.resolved_at IS NULL)
                ) AS direct_before,
        ...
        with self._get_connection() as conn:
            rows = conn.execute(
                ...
                WHERE entity_id = ? AND resolution_status = 'resolved'
                  AND lower(surface_form) = lower(?) AND resolution_method IN ({marks})
                ORDER BY id
                """,
                (entity_id, surface_form, *methods),
            ).fetchall()
