...
This is a Phase 0 stub. The schema is created by
``SqlitePodcastRepository._run_migrations`` (which already runs at app
boot); this class exists so Phase 1's extractor / resolver code can
import a typed surface today. Each operational method raises
``NotImplementedError`` until Phase 1 fills it in — keeping the
boundary explicit prevents accidental partial implementations from
landing.
"""
...
class SqliteEntityRepository:
    ...
    def insert_mentions(self, mentions: Iterable[EntityMention]) -> int:
        ...
        Phase 1: called by ``extract-entities`` once per episode with
        rows whose ``entity_id`` is ``None`` and ``resolution_status``
        is ``"pending"``.
        """
        raise NotImplementedError("Phase 1 — see spec #28 task 1.2")
