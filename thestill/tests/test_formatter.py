...
def main():
    ...
    transcript_path = Path(
        "data/transcripts/The_Prof_G_Pod_with_Scott_Galloway_How_to_AI-Proof_Your_Career,_Spot_Market_Hype,_and_Raise_Critical_Thinkers_—_ft._Greg_Shove_e884173f_transcript.json"
    )
    ...
    print(
        f"Reduction: {((len(json.dumps(transcript_data)) - len(markdown)) / len(json.dumps(transcript_data)) * 100):.1f}%"
    )
