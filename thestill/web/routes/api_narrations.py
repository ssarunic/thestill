...
"""Briefing narration artefact endpoints (spec #33, rekeyed to briefings).
...
a specific narration variant. The user-facing trigger lives at
``POST /api/briefings/{briefing_id}/narrate`` so the briefing record
stays the durable join key for narrations.
"""
...
def _resolve_owned_briefing(state: AppState, narration_id: str, user_id: str) -> Optional[Briefing]:
    ...
    Legacy digest-keyed narrations (pre-retirement artefacts under
    ``data/narrations/``) no longer resolve and 404 by design.
    """
    ...
    for n in range(len(parts) - 1, 0, -1):
        ...
        briefing = state.briefing_repository.get(candidate)
        if briefing is None:
            continue
