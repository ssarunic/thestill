...
const mockUseNarrateBriefing = useNarrateBriefing as ReturnType<typeof vi.fn>
...
beforeEach(() => {
  vi.clearAllMocks()
  mockUseNarrateBriefing.mockReturnValue({
    mutateAsync: vi.fn(),
    isPending: false,
    error: null,
  })
...
describe('NarrationView', () => {
  ...
  it('clicking a length chip that already exists swaps the displayed variant', async () => {
    ...
    expect(mockUseNarrateBriefing().mutateAsync).not.toHaveBeenCalled()
    expect(
  ...
  it('clicking a length chip that does not exist triggers narrateBriefing', async () => {
    ...
    mockUseNarrateBriefing.mockReturnValue({ mutateAsync, isPending: false, error: null })
    mockUseNarration.mockReturnValue({
