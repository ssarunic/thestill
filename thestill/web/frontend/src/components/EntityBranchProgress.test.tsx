...
import type { EpisodeTask } from '../api/types'

// Spec #68 D2 — tasks arrive as a prop; `EpisodeReader` owns the single
// task query, so there is no hook to mock here any more.
...
let currentTasks: EpisodeTask[] = []

function withTasks(tasks: Array<ReturnType<typeof task>>) {
  // One cast, here: the fixture carries only the fields the component reads.
  currentTasks = tasks as unknown as EpisodeTask[]
}
...
describe('EntityBranchProgress', () => {
  ...
  it('renders nothing when no entity-branch tasks exist for the episode', () => {
    ...
    const { container } = render(<EntityBranchProgress tasks={currentTasks} episodeId="ep1" />)
    expect(container.firstChild).toBeNull()
  })

  it('renders nothing when episodeId is null', () => {
    withTasks([])
    const { container } = render(<EntityBranchProgress tasks={currentTasks} episodeId={null} />)
    expect(container.firstChild).toBeNull()
  })

  it('renders the four entity-branch stages with the section header', () => {
    withTasks([task('extract-entities', 'processing')])
    render(<EntityBranchProgress tasks={currentTasks} episodeId="ep1" />)
  ...
  it('keeps the full strip visible when collapseWhenIdle is false', () => {
    ...
    render(<EntityBranchProgress tasks={currentTasks} episodeId="ep1" collapseWhenIdle={false} />)
