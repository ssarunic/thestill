...
class TranscriptFormatter:
    ...
    def format_transcript(self, transcript_data: Dict) -> str:
        ...
        """
        # Normalise and merge segments by speaker
        speaker_blocks = self._merge_segments_by_speaker(transcript_data.get("segments", []))

        # Build Markdown (content only, no header)
        return self._build_markdown(speaker_blocks)
    ...
    def _build_markdown(self, speaker_blocks: List[Tuple[float, str, str]]) -> str:
        """Build Markdown document from speaker blocks"""
        lines = []
    ...
    def format_to_file(self, transcript_json_path: str, output_md_path: str) -> str:
        """
        ...
        # Format to Markdown
        markdown = self.format_transcript(transcript_data)
