...
def get_audio_duration(audio_path: Union[str, Path]) -> Optional[int]:
    ...
    # spec #25, item 2.6: canonicalise the path before shelling out, and
    # validate the file actually starts with known audio magic bytes, so
    # an RSS-provided filename with shell metacharacters (or a polyglot
    # file) cannot influence what ffprobe interprets.
    try:
