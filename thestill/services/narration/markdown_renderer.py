...
class NarrationMarkdownRenderer:
    ...
    def _render_block(
        ...
        if block.kind in ("narration", "reaction"):
            ...
            if not text:
                return
            # A reaction is the anchor's one-line beat on the clip above
            # it (spec #77 Phase 2b); set it apart from the narration.
            out.append(f"*{text}*" if block.kind == "reaction" else text)
            out.append("")
            return
