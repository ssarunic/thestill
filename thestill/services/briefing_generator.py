...
"""
Briefing generator for creating morning briefing documents.

Implements THES-27: Generates a markdown briefing from processed episodes,
...
class BriefingEpisodeInfo:
    """Information about an episode for briefing generation."""
...
def extract_gist(summary_text: str) -> Optional[str]:
    ...
    Public so the narrated-briefing theme clusterer (spec #33) can feed
    the same compact episode synopsis into its LLM input that the
    link-index briefing renders into per-episode rows.
...
class BriefingGenerator:
    """
    Generates markdown briefing documents from processed episodes.

    The briefing provides a "morning briefing" view of recently processed
    ...
    Usage:
        generator = BriefingGenerator(path_manager, file_storage)
    ...
    def __init__(
        ...
        Args:
            ...
            file_storage: Spec #35 backend for briefing writes + summary reads.
    ...
    def generate(
        ...
        """
        Generate a briefing from processed episodes.
        ...
        Returns:
            BriefingContent with markdown and statistics
        """
        ...
        return BriefingContent(markdown=markdown, stats=stats)

    def write(self, content: BriefingContent, output_path: Path) -> Path:
        """
        Write briefing content to a file.

        Args:
            content: BriefingContent to write
        ...
        logger.info("Briefing written", path=str(output_path), backend_key=relative)
    ...
    def _generate_markdown(self, episode_infos: List[BriefingEpisodeInfo], stats: BriefingStats) -> str:
        """Generate the markdown content for the briefing."""
        lines = []
